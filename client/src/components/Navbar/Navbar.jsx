...
const Navbar = () => {
    ...
    if (user) {
        logout = <a href='/' class="text-red-900 px-3 py-2 rounded-md text-sm font-medium" onClick={() => { Logout() }}>logout</a>
        profile = <>
            <Link to="/profile"><img class="h-8 w-8 mr-2 rounded-full" src={user.img} alt="" /></Link>
            <Link class="hidden sm:block text-gray-900 px-3 py-2 rounded-md text-sm font-medium" to="/profile">{user.username} </Link>
        </>
        myblogs = <Link class="text-gray-900 px-3 py-2 rounded-md text-sm font-medium" to="/myblogs">My Blogs </Link>
        newblog = <Link class="text-gray-900 px-3 py-2 rounded-md text-sm font-medium" to="/newblog">New + </Link>

        navbar = <>
            <Link to="/myblogs" class="text-gray-900 px-3 py-2 rounded-md text-sm font-medium">Myblogs</Link>
            <Link to="/newblogs" class="text-gray-900 px-3 py-2 rounded-md text-sm font-medium">New +</Link>
            <Link to="/logout" class="text-gray-900 px-3 py-2 rounded-md text-sm font-medium">Logout</Link>
        </>
    }
    if (!user) {
        var login = <>
    ...
    return (
        <div>
            <nav class="bg-white w-full fixed top-0 z-10">
                    ...
                    <div class="relative flex items-center justify-between h-16">
                        ...
                        <div class="ml-8 sm:ml-0 flex-1 flex items-start sm:items-center sm:items-stretch sm:justify-start">
                            <Link to="/" class="flex">
                                ...
                                <h1 className="text-black font-bold font-serif tracking-widest mt-1 sm:mt-2 text-lg">INSTA BLOGS</h1>
                            </Link>
                            <div class="hidden sm:block sm:ml-6">
                                <div class="flex space-x-4 sm:mt-1">
                                    <Link to="/" class="text-black px-3 py-2 rounded-md text-sm font-medium" aria-current="page">Home</Link>
                                    {myblogs}
                                    {newblog}
                                    {logout}
                                </div>
